...
router.post('/', async(req, res)=> {
    ...
    userLogin = await User.findOne({ login: req.body.nickname})
    if (userLogin) return res.status(400).send('Login zajęty!');

    user = new User(_.pick(req.body, ['nickname','email','password','name','surname']));
...
function validate(req) {
    const schema = {
      nickname: Joi.string().min(5).max(30).required(), //dłuższy login?
